   ...
   for(;;) {
      ...
      switch( eir ) {
	    ...
	    //
            ...
            if( _q_size(_reading) > 0 ) {

               pcb = _q_remove( _reading );
	       if( pcb == NULL ) {
...
**
** usage:   int num = _sio_writes( const char *buffer, int length )
**
** returns the number of characters copied into the buffer
*/

int _sio_writes( const char *buffer, int length ) {
   int first = *buffer;
